...
mass theorem with charge $M\ge|q|/\sqrt2$ \cite{GKLTT,Nozawa,Rogatko,NozawaShiromizu}, and static uniqueness at $\alpha=1$
...
\begin{proposition}[Necessity of the source-free hypothesis; after McCormick \cite{McCormick}]\label{thm:nogo}
...
Section~\ref{sec:false} proves Proposition~\ref{thm:nogo}. Section~\ref{sec:outline} outlines the strategy for
...
large. We prove Proposition~\ref{thm:nogo} by an explicit construction.

\begin{proof}[Proof of Proposition~\ref{thm:nogo}]
...
\end{equation}
the first by $\Disc=0\Leftrightarrow W=1\Leftrightarrow R=1$ through \eqref{eq:discfac}. These are two
pointwise relations among the leaf variables and the momentum. The first, $R=1$, is one algebraic constraint
on $(t,u,\sigma)$; the second slaves $\phi'$ to the leaf. Substituting both into the master-saturated
constraint \eqref{eq:constraint} (with $\eps=0$) closes the system: it becomes a first-order autonomous ODE
for the profile, tangent to the surface $\{R=1\}$, whose right-hand side is determined by
\eqref{eq:rigidconds}. The GHS family \eqref{eq:ghs} satisfies both conditions of \eqref{eq:rigidconds} --- one
checks $R\equiv1$ and $G=\rho\,\Hbr\equiv\tfrac12 r_p=M$ identically in $\rho$ along it --- so it is an
integral curve; by uniqueness of solutions of the first-order system, it is the \emph{only} admissible one,
and the asymptotic data $t\to1,u\to0,\sigma\to1$ single out the GHS trajectory with the given $(\rH,q)$.
...
necessary (Proposition~\ref{thm:nogo}), whereas the sub-extremality condition $\rH\ge q$ is redundant
