...
\title{Proof of the Einstein--Maxwell--dilaton Penrose inequality\\ in spherical symmetry at $\alpha=1$}
...
Pythagorean form. We prove the conjecture, in its divergence-free form, for static, spherically symmetric,
time-symmetric data satisfying the dominant energy condition, with equality if and only if the data is GHS. The proof uses a quasilocal mass that
is nondecreasing along the area-radius foliation, equals $\tfrac12\sqrt{\rH^2+2q^2}$ at the horizon and tends
to $M$ at infinity; its monotonicity reduces to a polynomial positivity certificate. We show in addition that the divergence-free hypothesis $\operatorname{div}(e^{-2\phi}E)=0$ on the
Maxwell field is necessary --- transcribing McCormick's Einstein--Maxwell counterexample \cite{McCormick} to
EMD, where the dilaton weight cancels by Gauss's law --- whereas the sub-extremality condition $\rH\ge q$ often
imposed on charged Penrose inequalities is redundant here, and we give a second, independent proof of the bound
by a radical-free rational mass whose positivity certificate is verified in exact rational arithmetic.
\end{abstract}

\medskip
\noindent\textbf{Keywords:} Penrose inequality; Einstein--Maxwell--dilaton; dilaton black hole; quasilocal mass;
spherical symmetry.

\smallskip\noindent\textbf{Mathematics Subject Classification (2020):}
83C57 (Primary); 83C22, 53C80, 26C10 (Secondary).
...
cosmic censorship conjecture \cite{Penrose69,Penrose}. In the time-symmetric (Riemannian) case it was proved by Huisken
...
\end{equation*}
Khuri, Weinstein and Yamada \cite[Conjecture 1.7]{KWYext} conjectured that this bound holds for all EMD data
whose charge densities are compactly supported, with the Gibbons black hole \cite{Gibbons82} (the $\alpha=1$
member of the Gibbons--Maeda family, i.e.\ GHS) as the equality case when the fields are divergence-free. The
problem we solve is their conjecture in spherical symmetry.

One hypothesis needs care before it can be proved. As stated, compact support of the charge densities is not
enough: the bound then fails by an arbitrary factor, because a charge screened from the horizon by a distant
shell contributes to the asymptotic $q$ but not to the mass. McCormick \cite[Theorem 3.3]{McCormick}
established this for the Einstein--Maxwell case $\alpha=0$, and showed that a pointwise condition tying the
matter density to the charge density restores the inequality; Section~\ref{sec:false} transcribes his
construction to EMD, where it applies verbatim. The divergence-free hypothesis excludes exactly this
configuration, and it is the form we prove.

\begin{conjecture}[Khuri--Weinstein--Yamada \cite{KWYext}, divergence-free form]\label{conj:posed}
...
mass lower bounds in this setting (the Witten--Nester spinor identities \cite{Witten}, which give the positive
mass theorem with charge $M\ge|q|/\sqrt2$ \cite{GKLTT,Nozawa,Rogatko,NozawaShiromizu}, and static black-hole uniqueness
...
shows precisely what its absence costs. The mechanism is the one McCormick \cite{McCormick} identified for the
Einstein--Maxwell case $\alpha=0$: a charge screened from the horizon by a distant shell inflates the
asymptotic charge without adding mass. It transcribes to EMD unchanged --- the dilaton weight cancels from the
screening cost by Gauss's law --- so the following is McCormick's construction carried to $\alpha=1$.

\begin{proposition}[Necessity of the source-free hypothesis; after McCormick \cite{McCormick}]\label{prop:nogo}
...
The rational route is the one we ship as a machine-verifiable certificate: because it carries no radical, its
positivity is decided over $\mathbb Z$ by the Bernstein/Sturm/Handelman argument of Section~\ref{sec:rational}
and Appendix~\ref{app:sos}, reproduced in exact rational arithmetic by the supplementary script. Either proof
establishes Theorem~\ref{thm:main}; the radical route is the more direct conceptually, the rational route the
one a reader can rerun end to end.
...
instance $\alpha=2$). The correct sharp per-coupling bound is given by the GHS-calibrated
...
\cite{Nozawa,Rogatko,NozawaShiromizu}) and static black-hole uniqueness \cite[Theorem 1]{MarsSimon} --- proved
at $\alpha=1$, and for every coupling when the magnetic or the electric field vanishes --- are known; as noted
...
The Penrose inequality and the methods behind it are surveyed in \cite{Mars}, and the Riemannian theory is
developed systematically in \cite{LeeBook}. The monotone quasilocal mass used below descends from Geroch's
energy-extraction argument \cite{Geroch}, turned into a route to the Penrose inequality by Jang and Wald
\cite{JangWald} and made rigorous through inverse mean curvature flow \cite{HuiskenIlmanen} and Bray's conformal
flow \cite{Bray} (the latter extended below dimension eight in \cite{BrayLee}); the general, non-time-symmetric
conjecture is approached via the Jang equation by Bray and Khuri \cite{BrayKhuri}. The underlying mass--charge
positivity $M\ge|q|$ for black holes rests on the positive mass theorem \cite{SchoenYau,Witten} and its charged
black-hole form \cite{GHHP}, and a quasilocal charged Penrose inequality is established by Chen and McCormick
\cite{ChenMcCormick}; equality and rigidity for the relevant static black holes are governed by harmonic-map
uniqueness \cite{Weinstein,MarsSimon}. Against this background the sharp EMD bound with GHS rigidity proved here
appears to be new.

The paper is organized as follows. Section~\ref{sec:setup} fixes the data and the GHS solution, and
Section~\ref{sec:false} proves Proposition~\ref{prop:nogo}. Section~\ref{sec:outline} outlines the strategy for
...
This section fixes the initial-data class, writes the dominant energy condition as the single scalar constraint
used in the proof, and records the GHS solution that saturates the bound.
...
large. We prove Proposition~\ref{prop:nogo} by an explicit construction.

\begin{proof}[Proof of Proposition~\ref{prop:nogo}]
...
We now carry out the proof of Theorem~\ref{thm:main} along the plan of Section~\ref{sec:outline}, constructing
the mass and establishing in turn its flow identity, the two leaf inequalities, the endpoint values, and rigidity.
...
\subsection{Construction of the mass}\label{sec:massconstruct}

Formula \eqref{eq:Hbr} is not postulated; its three summands are produced in turn by three requirements
---calibration on the GHS reference flow, the horizon endpoint, and leaf positivity---the first two of which
determine their terms uniquely.

\emph{Step 1: the base term is the calibrated round Hawking mass.} Take the leading part of the mass to be
linear in the compactness $u$, as a Hawking mass is, $G_0=\rho\,(\alpha(t)+u\,\beta(t))$, and require it to
reproduce the GHS mass along the whole reference family \eqref{eq:ghs}: $G_0\equiv M=\tfrac12 r_p$. As the
GHS family is two-parameter, at each fixed $t$ this is two independent linear conditions on $(\alpha,\beta)$,
and it determines them uniquely,
\begin{equation}\label{eq:gammaconstruct}
   \alpha(t)=\frac{(t^2-1)^2}{2t(t^2+1)^2},\qquad
   \beta(t)=\frac{2t}{(t^2+1)^2},\qquad
   \gamma:=\alpha+u\beta=\frac{(t^2-1)^2+4u\,t^2}{2t(t^2+1)^2},
\end{equation}
the first term of \eqref{eq:Hbr}. At $t=1$ it collapses to $\gamma=u/2$, so $\rho\,\gamma=m$ is exactly the
round (uncharged) Hawking mass of the leaf; the dilaton dressing in \eqref{eq:gammaconstruct} is what the
calibration adds.

\emph{Step 2: the reference radical fixes the charge term.} The combination $R$ of \eqref{eq:W} is built so
that $R\equiv1$ on GHS; hence any correction proportional to $W-1=\sqrt R-1$ vanishes on the reference flow
and leaves the Step~1 calibration intact. Adding such a term, $\gamma+\tfrac{f}{2t}$, its shape is forced by
the horizon endpoint: at $u=1$ one has $\kappa=0$, $W=z=\sigma t$, and $\gamma(t,1)=\tfrac1{2t}$, so
\eqref{eq:Hend} requires $\tfrac1{2t}+\tfrac{f}{2t}=\tfrac{\sigma}{2}$, i.e.\ $f=\sigma t-1=W-1$, the second
term of \eqref{eq:Hbr}, determined uniquely. Terms one and two already carry the full endpoint and rigidity
content: they give $\Hbr(t,1,\sigma)=\sigma/2$ and $\Hbr(1,0,1)=0$, keep $G\equiv M$ on GHS, and yield the
factorisation $\Disc=-(W-1)^2P$ of \eqref{eq:discfac}, with the double root on $W=1$ that rigidity needs.

\emph{Step 3: the positivity completion.} The cofactor $P$ left by the first two terms is, however, not of
one sign, so $\Disc\le0$ can fail. One removes this by a third correction $-(1-u)(R-1)^2\,\Phi$, with
$\Phi=2t/[(W+z)(z+1)^2(t^2+1)^2]$, the third term of \eqref{eq:Hbr}. Its two prefactors are structural: the
$(1-u)$ makes it vanish at the horizon, and $(R-1)^2=(W-1)^2(W+1)^2$ makes it vanish on GHS and preserve the
double root of Step~2, so it perturbs none of the endpoint, calibration, or rigidity identities. Its sole
role, and that of the rational factor $\Phi$, is to render the cofactor nonnegative, after which $\Disc\le0$
carries the certificate of Section~\ref{sec:disc}. Unlike Steps~1--2 this last term is \emph{selected} by
demanding leaf positivity rather than fixed by a reference value; it is the one fitted, rather than forced,
ingredient of \eqref{eq:Hbr}. Each identity above (uniqueness of $\gamma$; $R\equiv1$ and $G\equiv M$ on GHS;
$f=W-1$; the endpoints $\Hbr(1,0,1)=0$, $H_u(1,0,1)=\tfrac12$; and $\Disc=-(W-1)^2P$) is verified in exact
...
On the admissible range $t>0,\ u\in[0,1),\ \sigma\ge1$ one has $H_u>0$ and $\Disc\le0$, with $\Disc=0$
...
\]
Here $D_2>0$ on the whole range $t>0$: for $t\le1$ the term $2(t^2-1)(u-1)$ is a product of two nonpositive
factors, hence $\ge0$, so $D_2\ge\sigma^2(t^2+1)^2>0$; for $t\ge1$ it is bounded below by $-2(t^2-1)$, and
$\sigma^2(t^2+1)^2\ge(t^2+1)^2>2(t^2-1)$ since $(t^2+1)^2-2(t^2-1)=t^4+3>0$, so again $D_2>0$. The
...
$\{t>0,\,u\in[0,1),\,\sigma\ge1\}$ has $H_u\le0$, so $H_u>0$ unconditionally, for every $\sigma\ge1$
...
This completes the proof of Lemma~\ref{lem:psd}.
...
$C$, to which the Bernstein/Sturm certificate applies. The cofactor $C$ and its coefficient family $P_k,Q_k$
are large, and we do not print them; the reader who wants a certificate to rerun is directed to the
radical-free rational proof of Section~\ref{sec:rational}, whose (smaller) polynomials $B_9,M_9$ and their
nonnegativity certificate are supplied as the machine-verifiable supplementary bundle. That proof establishes
the same Theorem~\ref{thm:main} independently.
...
At infinity the data flatten, with fall-off $u=2m/\rho\to0$ ($u=2M/\rho+o(1/\rho)$), $t-1=e^\phi-1=O(1/\rho)$
(the dilaton charge), and $\sigma-1=q^2/\rho^2+O(\rho^{-4})=O(1/\rho^2)$. Since $t-1$ is the \emph{same}
order as $u$, the limit needs the full first-order expansion of $\Hbr$ about $(1,0,1)$,
\[
  \Hbr=\Hbr(1,0,1)+H_u(1,0,1)\,u+H_t(1,0,1)\,(t-1)+H_\sigma(1,0,1)\,(\sigma-1)+o(1/\rho).
\]
One computes $\Hbr(1,0,1)=0$, $H_u(1,0,1)=\tfrac12$, and --- crucially --- $H_t(1,0,1)=0$, so the
$O(1/\rho)$ dilaton-charge term drops out and the $H_\sigma$ term is $O(1/\rho^2)$; hence $\rho\,\Hbr\to
\tfrac12\cdot2M=M$, i.e.
\begin{equation}\label{eq:Ginf}
  G(\infty)=\lim_{\rho\to\infty}\rho\,\Hbr=M .
\end{equation}
Were $H_t(1,0,1)\ne0$, a nonzero dilaton charge would contaminate the ADM mass; the identity
$H_t(1,0,1)=0$ is what makes $G$ calibrated to $M$ at infinity.
...
Equality in Theorem~\ref{thm:main} forces $\dd G/\dd\rho\equiv0$ at \emph{every} leaf, hence $\eps\equiv0$ and
the quadratic $A_2p^2+A_1p+A_0$ vanishes identically. As $A_2>0$ and $\Disc\le0$, this requires both
$\Disc=0$ and that the momentum sit at the double root, so along an equality profile
\begin{equation}\label{eq:rigidconds}
  R(t,u,\sigma)\equiv1\qquad\text{and}\qquad p=\rho\,\phi'=-\frac{A_1}{2A_2}=-\frac{t\,H_t}{2(1-u)H_u} ,
\end{equation}
the first by $\Disc=0\Leftrightarrow W=1\Leftrightarrow R=1$ through \eqref{eq:discfac}. Substituting both
into the master-saturated constraint \eqref{eq:constraint} (with $\eps=0$) closes the system: with
$q^2/\rho^2=(\sigma^2-1)/2$ and $\rho\,\dd/\dd\rho$ as the flow derivative, an equality profile solves the
autonomous system
\begin{equation}\label{eq:rigidode}
  \rho\,\frac{\dd t}{\dd\rho}=t\,p,\qquad
  \rho\,\frac{\dd u}{\dd\rho}=\frac{(\sigma^2-1)t^2}{2}+(1-u)\,p^2-u,\qquad
  \rho\,\frac{\dd\sigma}{\dd\rho}=-\frac{\sigma^2-1}{\sigma},
\end{equation}
with $p$ given by \eqref{eq:rigidconds}. Two facts make this a genuine initial-value problem \emph{at the
horizon}, where both $A_1$ and $A_2$ vanish.

First, the slaved momentum extends analytically across $u=1$. By \eqref{eq:Hend} the value
$\Hbr(t,1,\sigma)=\sigma/2$ does not depend on $t$, so $H_t$ vanishes identically on $\{u=1\}$ and factors as
$H_t=(u-1)\,\mathcal N$ with $\mathcal N$ analytic there --- at $u=1$ one has $\kappa=0$, hence $R=z^2>0$ and $W+z=2\sigma t>0$,
so $W=\sqrt R$ has no branch point. The factor $1-u$ therefore cancels, and
\begin{equation}\label{eq:pana}
  p=-\frac{t\,H_t}{2(1-u)H_u}=\frac{t\,\mathcal N}{2H_u},\qquad
  p\big|_{u=1}=\frac{t\,H_{tu}}{2H_u}\bigg|_{u=1}=\frac{1-t^2}{1+t^2},
\end{equation}
a removable $0/0$. With $H_u|_{u=1}=t^2(\sigma^2+1)/\big(\sigma(t^2+1)^2\big)>0$, the right-hand side of
\eqref{eq:rigidode} is analytic on a neighbourhood of the horizon point in $\{t>0,\ \sigma>1\}$.

Second, the horizon data are \emph{determined by $(\rH,q)$ alone}. At $u=1$ the rigidity constraint reads
$R=\sigma^2t^2=1$, so $\sigma t=1$: the horizon dilaton value, otherwise free, is pinned to
\begin{equation*}
  t_H=\frac{1}{\sigma_H},\qquad \sigma_H=\sqrt{1+2q^2/\rH^2},
\end{equation*}
and then \eqref{eq:pana} gives $p_H=(\sigma_H^2-1)/(\sigma_H^2+1)=q^2/(\rH^2+q^2)$. The GHS family
\eqref{eq:ghs} satisfies all of this: $R\equiv1$ and $G=\rho\,\Hbr\equiv\tfrac12 r_p=M$ identically in
$\rho$ along it, $t_H\sigma_H=1$ identically, and $p=r_m/(2r-r_m)$ agrees with \eqref{eq:pana}. Hence GHS is
an integral curve of \eqref{eq:rigidode} through the point $(t,u,\sigma)=(1/\sigma_H,1,\sigma_H)$, and by
Picard--Lindel\"of that orbit is the only one. An equality profile with horizon radius $\rH$ and charge $q$
therefore coincides with it. Thus $G$ is constant exactly on GHS, and equality holds only there. This
...
The flow identity \eqref{eq:flow} holds verbatim for every $F=F(t,u,\sigma)$, so on $\mathcal A$ one has
...
\end{equation}
an identity in which every coefficient is nonnegative \cite{Handelman,PowersReznick,Polya}; as $t,w>0$ and $B_{3,k}\ge0$,
...
necessary (Proposition~\ref{prop:nogo}), whereas the sub-extremality condition $\rH\ge q$ is redundant
...
Two directions remain. For non-spherical data the analogous monotonicity is open; the current evidence is a
convergent numerical flow with no violation down to $\rH/q=0.63$. For a general coupling $\alpha\ne1$, which
Khuri, Weinstein and Yamada explicitly left for future work \cite{KWYext}, the leaf-by-leaf monotonicity used
here is obstructed: the discriminant of Section~\ref{sec:disc} changes sign. A companion paper
\cite{ItkinGeneralAlpha} bypasses that obstruction by treating the inequality variationally, and proves,
uniformly in $\alpha$, that the GHS profile is a strict local minimiser of the mass; the passage from local to
global there remains open, and for $\alpha=1$ it is exactly the content the present paper supplies. The
present paper settles the spherically symmetric case and removes the charge--horizon hypothesis from it.
...
\textbf{Funding.} No funding was received for this work. \textbf{Competing interests.} The author declares no
competing interests. \textbf{Ethics approval.} Not applicable: this is a theoretical study involving no human
or animal subjects, data, or tissue.

\section*{Data availability}
The manuscript source and a machine-verifiable supplementary bundle (a \texttt{sympy} script that re-derives, in
exact rational arithmetic, every algebraic claim of Section~\ref{sec:ratcert} and Appendix~\ref{app:sos},
together with the polynomials $B_9,M_9$ and the Handelman certificate for $M_9$) are available in the author's
repository \url{https://github.com/YehudaItkin/spherical-emd-penrose} and will be archived with a
persistent DOI (Zenodo) on submission.

\begin{thebibliography}{36}
\bibitem{Penrose} Penrose R 1973 Naked singularities \emph{Ann. New York Acad. Sci.} \textbf{224} 125--134
\bibitem{MisnerSharp} Misner C W and Sharp D H 1964 Relativistic equations for adiabatic, spherically
  symmetric gravitational collapse \emph{Phys. Rev.} \textbf{136} B571--B576
\bibitem{Gibbons82} Gibbons G W 1982 Antigravitating black hole solitons with scalar hair in $N=4$
  supergravity \emph{Nucl. Phys.} B \textbf{207} 337--349
\bibitem{GibbonsMaeda} Gibbons G W and Maeda K 1988 Black holes and membranes in higher-dimensional
  theories with dilaton fields \emph{Nucl. Phys.} B \textbf{298} 741--775
\bibitem{GKLTT} Gibbons G W, Kastor D, London L A J, Townsend P K and Traschen J 1994 Supersymmetric
  self-gravitating solitons \emph{Nucl. Phys.} B \textbf{416} 850--880 (\emph{Preprint} arXiv:hep-th/9310118)
\bibitem{ItkinGeneralAlpha} Itkin I 2026 The spherically symmetric Einstein--Maxwell--dilaton Penrose
  inequality for a general coupling: strict local minimality of the GHS profile, uniform in $\alpha$
  \emph{Preprint}
\bibitem{GHS} Garfinkle D, Horowitz G T and Strominger A 1991 Charged black holes in string theory
  \emph{Phys. Rev.} D \textbf{43} 3140--3143 (Erratum 1992 \emph{Phys. Rev.} D \textbf{45} 3888)
\bibitem{HuiskenIlmanen} Huisken G and Ilmanen T 2001 The inverse mean curvature flow and the Riemannian
  Penrose inequality \emph{J. Diff. Geom.} \textbf{59} 353--437
\bibitem{Bray} Bray H L 2001 Proof of the Riemannian Penrose inequality using the positive mass theorem
  \emph{J. Diff. Geom.} \textbf{59} 177--267
\bibitem{DisconziKhuri} Disconzi M M and Khuri M A 2012 On the Penrose inequality for charged black holes
  \emph{Class. Quantum Grav.} \textbf{29} 245019
\bibitem{KWYext} Khuri M, Weinstein G and Yamada S 2015 Extensions of the charged Riemannian Penrose
  inequality \emph{Class. Quantum Grav.} \textbf{32} 035019 (\emph{Preprint} arXiv:1410.5027)
\bibitem{KWY} Khuri M, Weinstein G and Yamada S 2017 Proof of the Riemannian Penrose inequality with
  charge for multiple black holes \emph{J. Diff. Geom.} \textbf{106} 451--498
\bibitem{McCormick} McCormick S 2020 On the charged Riemannian Penrose inequality with charged matter
  \emph{Class. Quantum Grav.} \textbf{37} 015007 (\emph{Preprint} arXiv:1907.07967)
\bibitem{Kunduri} Kunduri H K, Margalef-Bentabol J and Muth S 2025 The Penrose inequality in spherical
  symmetry with charge and in Gauss--Bonnet gravity \emph{J. Math. Phys.} \textbf{66} 052502 (\emph{Preprint} arXiv:2409.07639)
\bibitem{McCormickWeighted} McCormick S 2026 Mass, staticity, and a Riemannian Penrose inequality for
  weighted manifolds \emph{SIGMA} \textbf{22} 041 (\emph{Preprint} arXiv:2601.19875)
\bibitem{NSIY} Nozawa M, Shiromizu T, Izumi K and Yamada S 2018 Divergence equations and uniqueness
  theorem of static black holes \emph{Class. Quantum Grav.} \textbf{35} 175009 (\emph{Preprint} arXiv:1805.11385)
\bibitem{Nozawa} Nozawa M 2011 On the Bogomol'nyi bound in Einstein--Maxwell--dilaton gravity
  \emph{Class. Quantum Grav.} \textbf{28} 175013 (\emph{Preprint} arXiv:1011.0261)
\bibitem{Rogatko} Rogatko M 2000 Positive mass theorem for black holes in Einstein--Maxwell axion--dilaton
  gravity \emph{Class. Quantum Grav.} \textbf{17} 11--17 (\emph{Preprint} arXiv:hep-th/9910231)
\bibitem{NozawaShiromizu} Nozawa M and Shiromizu T 2014 Positive mass theorem in extended supergravities
  \emph{Nucl. Phys.} B \textbf{887} 380--399 (\emph{Preprint} arXiv:1407.3355)
\bibitem{MarsSimon} Mars M and Simon W 2002 On uniqueness of static Einstein--Maxwell--dilaton black
  holes \emph{Adv. Theor. Math. Phys.} \textbf{6} 279--305 (\emph{Preprint} arXiv:gr-qc/0105023)
\bibitem{Handelman} Handelman D 1988 Representing polynomials by positive linear functions on compact convex
  polyhedra \emph{Pacific J. Math.} \textbf{132} 35--62
\bibitem{PowersReznick} Powers V and Reznick B 2000 Polynomials that are positive on an interval
  \emph{Trans. Am. Math. Soc.} \textbf{352} 4677--4692
\bibitem{Polya} P\'olya G 1928 \"Uber positive Darstellung von Polynomen \emph{Vierteljschr. Naturforsch.
  Ges. Z\"urich} \textbf{73} 141--145
\bibitem{BasuPollackRoy} Basu S, Pollack R and Roy M-F 2006 \emph{Algorithms in Real Algebraic Geometry}
  (Algorithms and Computation in Mathematics vol 10) 2nd edn (Berlin: Springer)
\bibitem{Penrose69} Penrose R 1969 Gravitational collapse: the role of general relativity
  \emph{Riv. Nuovo Cimento} \textbf{1} 252--276 (reprinted 2002 \emph{Gen. Relativ. Gravit.} \textbf{34} 1141--1165)
\bibitem{Mars} Mars M 2009 Present status of the Penrose inequality \emph{Class. Quantum Grav.} \textbf{26}
  193001 (\emph{Preprint} arXiv:0906.5566)
\bibitem{LeeBook} Lee D A 2019 \emph{Geometric Relativity} (Graduate Studies in Mathematics vol 201)
  (Providence, RI: American Mathematical Society)
\bibitem{Geroch} Geroch R 1973 Energy extraction \emph{Ann. New York Acad. Sci.} \textbf{224} 108--117
\bibitem{JangWald} Jang P S and Wald R M 1977 The positive energy conjecture and the cosmic censor hypothesis
  \emph{J. Math. Phys.} \textbf{18} 41--44
\bibitem{SchoenYau} Schoen R and Yau S-T 1979 On the proof of the positive mass conjecture in general
  relativity \emph{Commun. Math. Phys.} \textbf{65} 45--76
\bibitem{Witten} Witten E 1981 A new proof of the positive energy theorem \emph{Commun. Math. Phys.}
  \textbf{80} 381--402
\bibitem{GHHP} Gibbons G W, Hawking S W, Horowitz G T and Perry M J 1983 Positive mass theorems for black
  holes \emph{Commun. Math. Phys.} \textbf{88} 295--308
\bibitem{ChenMcCormick} Chen P-N and McCormick S 2022 Quasi-local Penrose inequalities with electric charge
  \emph{Int. Math. Res. Not. IMRN} \textbf{2022} 17333--17362 (\emph{Preprint} arXiv:2002.04557)
\bibitem{BrayKhuri} Bray H L and Khuri M A 2010 A Jang equation approach to the Penrose inequality
  \emph{Discrete Contin. Dyn. Syst.} \textbf{27} 741--766 (\emph{Preprint} arXiv:0910.4785)
\bibitem{BrayLee} Bray H L and Lee D A 2009 On the Riemannian Penrose inequality in dimensions less than eight
  \emph{Duke Math. J.} \textbf{148} 81--106 (\emph{Preprint} arXiv:0705.1128)
\bibitem{Weinstein} Weinstein G 1996 $N$-black-hole stationary and axially symmetric solutions of the
  Einstein--Maxwell equations \emph{Commun. Partial Differ. Equ.} \textbf{21} 1389--1430
  (\emph{Preprint} arXiv:gr-qc/9412036)
\end{thebibliography}
