...
and the redundancy of the charge--horizon hypothesis}
...
\begin{abstract}
The Penrose inequality bounds the total mass of a black-hole spacetime below by the area of its horizon,
and its sharp charge-dependent refinements test cosmic censorship in the presence of matter. For the
Einstein--Maxwell--dilaton (EMD) system with dilaton coupling $\alpha=1$, Khuri, Weinstein and Yamada
\cite{KWYext} conjectured the sharp bound $2M\ge\sqrt{\rH^2+2q^2}$, where $M$ is the ADM (Arnowitt--Deser--Misner)
mass, $\rH$ the horizon area radius and $q$ the charge; it is saturated by the Gibbons--Maeda /
Garfinkle--Horowitz--Strominger (GHS) black hole. The available positive-mass and spinorial methods produce
only an additive charge term and do not reach this Pythagorean form. We prove the conjecture for static,
spherically symmetric, time-symmetric data satisfying the dominant energy condition (DEC), with equality if
and only if the data is GHS. The proof constructs a quasilocal mass that is nondecreasing along the
area-radius foliation, equals $\tfrac12\sqrt{\rH^2+2q^2}$ at the horizon, and tends to $M$ at infinity; its
radial derivative is a quadratic in the dilaton momentum whose nonpositive discriminant reduces, after
rationalizing a single nested radical, to a polynomial positivity certificate verified in exact arithmetic.
We show in addition that the divergence-free hypothesis $\operatorname{div}(e^{-2\phi}E)=0$ on the Maxwell
field is necessary: without it, a charge supported on a shell outside the horizon drives the mass to the
uncharged value $\rH/2$, violating the bound by an arbitrary factor. By contrast, the sub-extremality condition
$\rH\ge q$ frequently imposed on charged Penrose inequalities is redundant here. We give a second, independent
proof of the bound by a radical-free rational mass of different algebraic type, which serves as a cross-check.
\end{abstract}
